#!/usr/bin/env python3
"""
generate_weekly_leaders_report.py — WAT Tool
Reads .tmp/weekly_tasks.json and generates weekly_leaders_report.html.
Same as generate_weekly_report.py, but with extra cards for the design lead
team: an Emily Forehand card, plus grouped cards for "Design Extra" and
...
OUTPUT_FILE = ROOT / "weekly_leaders_report.html"

# Each entry: (display_label, [username substrings, lowercase])
# A task is matched to a group if any of its assignees' usernames contains one
# of the listed substrings. Order is preserved in the output.
TEAM_GROUPS = [
    ("Bella",            ["bella"]),
    ("Caleb Chunn",      ["caleb chunn"]),
    ...
    ("Emily Forehand",   ["emily forehand"]),
    ("Design Extra",     ["dustin brand", "eli meneses", "ben forehand"]),
    ("External",         ["parker vanblaricom", "zach vanblaricom",
                          "caleb trinidad", "johnny hill"]),
]

# When a task is shared between this group and another, hide it from this
# group's column to avoid duplicate cards.
DEDUP_WHEN_SHARED = {"Olivia McCrimon"}
...
def assignee_chip(task, group_tokens):
    """For grouped (multi-person) cards, show the matching assignee's name."""
    for a in (task.get("assignees") or []):
        username = (a.get("username") or "").strip()
        if not username:
            continue
        if any(tok in username.lower() for tok in group_tokens):
            display = username.title() if username.islower() else username
            return f'<span class="assignee-chip">{esc(display)}</span>'
    return ""


# ── HTML builders ─────────────────────────────────────────────────────────────

def task_row_html(task, subtasks, week_start_date, is_sub=False, group_tokens=None):
    ...
    if subtasks and not is_sub:
        sub_html = "".join(
            task_row_html(s, [], week_start_date, is_sub=True, group_tokens=group_tokens)
            for s in subtasks
        )

    context_html = f'<span class="task-context">{esc(context)}</span>' if context else ""
    chip_html = assignee_chip(task, group_tokens) if group_tokens and len(group_tokens) > 1 else ""
...
def member_card_html(member_name, overdue_tasks, this_week_tasks, subtask_map,
                     week_start_date, group_tokens=None):
    ...
    is_group = bool(group_tokens) and len(group_tokens) > 1
    member_count_label = (f' &middot; {len(group_tokens)} people' if is_group else "")
    ...
    def task_rows(task_list):
        ...
        for t in sorted(task_list, key=lambda x: int(x.get("due_date") or 0)):
            ...
            rows += task_row_html(t, subs, week_start_date, group_tokens=group_tokens)
        return rows
    ...
    group_class = " group-card" if is_group else ""
    return f"""
  <div class="member-card{group_class}">
    <div class="member-header">
      <div class="avatar">{initials}</div>
      <div class="member-name">{esc(member_name)}{member_count_label}</div>
...
def combined_section_html(combined_by_member, overdue_by_member, this_week_by_member,
                           subtask_map, week_start_date, week_label, group_tokens_map):
    ...
    cards = "".join(
        member_card_html(
            ...
            subtask_map,
            week_start_date,
            group_tokens=group_tokens_map.get(name),
        )
  ...
  <section class="report-section">
    <div class="section-header">
      ...
      <div>
        ...
        <div class="section-meta">{total} task{"s" if total != 1 else ""} &middot; {len(combined_by_member)} card{"s" if len(combined_by_member) != 1 else ""}</div>
      </div>
    </div>
...
def build_html(overdue_by_member, this_week_by_member, week_start, week_end,
    ...
    group_tokens_map = {label: tokens for label, tokens in TEAM_GROUPS}

    # Preserve TEAM_GROUPS order in the output
    ordered_names = [label for label, _ in TEAM_GROUPS
                     if label in overdue_by_member or label in this_week_by_member]
    combined_by_member = {
    ...
    section = combined_section_html(
        ...
        subtask_map, week_start, week_label, group_tokens_map
    )
...
<head>
  ...
  <title>Weekly Leaders Report — The Life Church Design</title>
...
<body>
  ...
  <header class="site-header">
    <div class="header-eyebrow">The Life Church · Design Memphis · Leaders</div>
    <h1 class="header-title">Weekly Leaders Report</h1>
  ...
  <footer class="site-footer">
    <p>The Life Church Design Memphis &middot; Weekly Leaders Report</p>
...
def main():
    ...
    def task_group_labels(task):
        """Return the set of TEAM_GROUPS labels this task belongs to."""
        labels = []
        for label, tokens in TEAM_GROUPS:
            for a in (task.get("assignees") or []):
                uname = (a.get("username") or a.get("email") or "").lower()
    ...
    def is_team_task(task):
        return bool(task_group_labels(task))
    ...
    # Group tasks by TEAM_GROUPS label. Labels in DEDUP_WHEN_SHARED are dropped
    # from a task when another group is also assigned, to avoid duplicate cards.
    def group_by_member(task_list):
        groups = {}
        for t in task_list:
            labels = task_group_labels(t)
            if len(labels) > 1:
                labels = [l for l in labels if l not in DEDUP_WHEN_SHARED] or labels
            for label in labels:
